...
# Main function
def collect(
  extra_files=[], output_dirname='3-nvemd', input_dirname=RESTART_DIRNAME,
  ...
  collected_xyz = os.path.join(output_dirname, XYZ_FILENAME) # modi1

  # Recursively append restart MD results
  current_iter = 0
  collected_frames = []

  # Read existing collected frames if add-mode is True
  ...
  while True:
    ...
    root, dirs, files = next(os.walk('3-nvemd'))
    for d in dirs:
      os.chdir(d) 
      iter_from = load_iter_range(ITER_FILENAME)["from"] # Append frames
