...
def main():
    ...
    parser.add_argument("-f", "--force", type=float, help="Converge if forces on all atoms < fmax; Defaults to 1e-2 (DFTB can be set to 1e-3!") # Add an argument: force
    ...
    parser.add_argument("-d", "--dispersion",action='store_true',default=True, help="Apply DFT-D3 dispersion? (True/False); Defaults to True!") # Add an argument: dispersion
    parser.add_argument("-m", "--mesh",type=int,default=[8,8,8], help="The mesh grid running phonopy mesh, Defaults to [8,8,8]") # Add an argument: mesh
    ...
    if args.workflow == 1:
        ...
        print(" ----------------------------------------------------------------- ")
        workflow.dftfd(kpts=args.kpts, disp=args.dispersion, fmax=args.force, mesh=args.mesh, supercell=args.supercell)
    ...
    if args.workflow == 3: 
        ...
        print(" ----------------------------------------------------------------- ")
        workflow.dftmd(disp=args.dispersion, fmax=args.force, kpts=args.kpts, nsw1=args.nsw1, nsw2=args.nsw2, supercell=args.supercell, tebeg=args.temp1, teend=args.temp2)
